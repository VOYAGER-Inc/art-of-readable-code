...
Kiểm thử có nghĩa là những điều khác nhau đối với những người khác nhau. Trong chương này, chúng tôi sử dụng "kiểm thử" có nghĩa là bất kỳ mã nào có mục đích duy nhất là kiểm thử hành vi của một đoạn mã ("thực") khác. Chúng tôi sẽ tập trung vào khía cạnh dễ đọc của các bài kiểm thử chứ không đi sâu vào việc liệu bạn có nên viết mã kiểm thử trước khi viết mã thực ("phát triển hướng kiểm thử") hay các khía cạnh triết học khác của phát triển kiểm thử hay không.

## Làm cho các bài kiểm thử dễ đọc và dễ bảo trì
Điều quan trọng đối với mã kiểm thử là có thể đọc được cũng như đối với mã không kiểm thử. Các lập trình viên khác thường xem mã kiểm thử như là tài liệu không chính thức về cách mã thực hoạt động và nên được sử dụng. Vì vậy, nếu các bài kiểm thử dễ đọc, người dùng sẽ hiểu rõ hơn về cách thức hoạt động của mã thực.
...
Theo nguyên tắc thiết kế chung, bạn nên giấu những chi tiết ít quan trọng hơn từ phía người dùng, thay vì đó hãy để những chi tiết quan trọng hơn trở nên nổi bật nhất.
...
```
Sử dụng chức năng này, mã kiểm thử của chúng tôi trở nên rút gọn hơn một chút:
...
```
Nhưng điều này vẫn chưa đủ tốt - vẫn còn những chi tiết không quan trọng trong vấn đề của chúng ta. Ví dụ: tham số "http://example.com" thật chướng mắt. Nó luôn giống nhau và URL chính xác thậm chí không quan trọng — bạn chỉ cần điền vào ScoredDocument. Một chi tiết không quan trọng khác mà chúng tôi buộc phải xem là docs.resize (5) và & docs [0], & docs [1], v.v. Hãy thay đổi chức năng trợ giúp của chúng tôi để thực hiện nhiều công việc hơn cho chúng tôi và gọi nó là AddScoredDoc ():
```python
...
```
Sử dụng chức năng này, mã kiểm thử của chúng tôi thậm chí còn được rút gọn hơn:
```python
  void Test1() {
    vector<ScoredDocument> docs;
...
## Tạo tuyên bố kiểm thử tối thiểu
...
ASSERT() TỐT HƠN TRONG CÁC NGÔN NGỮ KHÁC
...
```
Những giá trị này không cần thiết phải phức tạp. (Và họ thậm chí không kiểm tra mã kỹ lưỡng.)
...
Các giá trị khác trong thử nghiệm của chúng tôi không quá tệ, nhưng khi chúng tôi ở đây, chúng tôi có thể giảm chúng xuống các số nguyên đơn giản nhất có thể. Ngoài ra, chỉ cần một giá trị âm để kiểm tra xem các giá trị âm có bị loại bỏ hay không. Đây là phiên bản kiểm thử mới của chúng tôi:
```python
...
Thay vì xây dựng một đầu vào “hoàn hảo” duy nhất để thực hiện kỹ lưỡng mã của bạn, việc viết nhiều thử nghiệm nhỏ hơn thường dễ dàng hơn, hiệu quả hơn và dễ đọc hơn.
...
Lưu ý rằng nếu bạn đang sử dụng frameworks kiểm thử, có thể đã có các quy tắc hoặc quy ước về cách đặt tên các phương pháp. Ví dụ: mô-đun đơn nhất Python mong đợi các tên phương pháp kiểm thử bắt đầu bằng “test”.
Khi nói đến việc đặt tên cho các hàm của trình trợ giúp trong mã thử nghiệm của bạn, bạn nên làm nổi bật liệu hàm có tự xác nhận hay không hay chỉ là một trình trợ giúp thông thường “kiểm thử không biết”. Ví dụ, trong chương này, bất kỳ hàm trợ giúp nào gọi assert() được đặt tên là Check... (). Nhưng chức năng
AddScoredDoc() được đặt tên giống như một hàm trợ giúp thông thường.
...
```
Bây giờ chúng ta đã học được một số kỹ thuật để viết các bài kiểm thử tốt hơn, hãy xác định chúng:
1. Bài kiểm thử rất dài và đầy những chi tiết không quan trọng. Bạn có thể mô tả những gì bài kiểm thử này đang thực hiện trong một câu, do đó, câu lệnh kiểm thử không nên dài hơn nhiều.
...
3. Thông báo lỗi kiểm thử không hữu ích lắm. Nếu kiểm thử này không thành công, nó sẽ chỉ thông báo Assertion failed: docs.size() == 3, điều này không cung cấp cho bạn đủ thông tin để gỡ lỗi thêm.
...
Mặt khác, nếu bạn có một thiết kế dễ viết kiểm thử thì đó là một dấu hiệu tốt.
Bảng 14-2 liệt kê một số đặc điểm thiết kế và thử nghiệm có lợi.
...
- Bị ám ảnh về phạm vi kiểm thử 100%. Kiểm thử 90% mã đầu tiên của bạn thường ít công việc hơn kiểm tra 10% cuối cùng. 10% cuối cùng đó có thể liên quan đến giao diện người dùng hoặc các trường hợp lỗi ngớ ngẩn, trong đó chi phí của lỗi không thực sự cao và nỗ lực để kiểm tra nó là không đáng. Sự thật là bạn sẽ không bao giờ nhận được 100% mức độ phù hợp. Nếu đó không phải là một lỗi bị bỏ sót, nó có thể là một tính năng bị bỏ lỡ hoặc bạn có thể không nhận ra rằng thông số kỹ thuật nên được thay đổi.
Tùy thuộc vào mức độ tốn kém của các lỗi của bạn, có một điểm chính xác về thời gian phát triển mà bạn nên dành cho mã kiểm thử. Nếu bạn đang xây dựng một nguyên mẫu trang web, thì việc viết bất kỳ mã thử nghiệm nào có thể không có giá trị. Mặt khác, nếu bạn đang viết bộ điều khiển cho tàu vũ trụ hoặc thiết bị y tế, thì kiểm thử có lẽ là trọng tâm chính của bạn.
- Để thử nghiệm cản trở quá trình phát triển sản phẩm. Chúng tôi đã thấy các tình huống trong đó kiểm thử, vốn chỉ là một khía cạnh của dự án, chi phối toàn bộ dự án. Kiểm thử trở thành một loại thần thánh nào đó được xoa dịu và các lập trình viên chỉ cần thực hiện các nghi lễ và chuyển động mà không nhận ra rằng thời gian kỹ thuật quý giá của họ có thể được dành cho nơi khác tốt hơn.
